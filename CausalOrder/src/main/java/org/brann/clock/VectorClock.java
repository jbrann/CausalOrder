...
//				jp.nextToken(); // past end object
    ...
    public synchronized void merge(VectorClock other) {
        ...
        for (Iterator otherIt = other.clocks.keySet().iterator();
        otherIt.hasNext();) {
            
            String nm = (String)otherIt.next();
            if (clocks.containsKey(nm)) {
                if (((LogicalClock)clocks.get(nm)).isLessThan(
    ...
    public Object clone() {
        ...
        for (Iterator myIt = this.clocks.keySet().iterator();
             myIt.hasNext();) {
            
            wknm = (String) myIt.next();
    ...
    protected boolean isLessThan(ClockOperations other) {
        ...
        for (Iterator MyIt = clocks.keySet().iterator();
        MyIt.hasNext();) {
            
            nm = (String) MyIt.next();
            wk = (LogicalClock) clocks.get(nm);
            otherWk = (LogicalClock) ((VectorClock)other).clocks.get(nm);
        ...
        if (!lessThan) {
            for (Iterator otherIt = ((VectorClock)other).clocks.keySet().iterator();
                 !lessThan &&
                 otherIt.hasNext();) {
            
                nm = (String) otherIt.next();
    ...
    public void tick(String name) {
        LogicalClock toTick = (LogicalClock)clocks.get(name);
    ...
    private static final String CLOCK_SEP = "|";
