 ...
 *
 * A VectorClock is a set of name/value pairs where the name identifies a
 * process in a distributed environment and the value is a LogicalClock.
 *
 * @author John Brann
 * @see LogicalClock
...
public class VectorClock extends ClockOperations implements Serializable,
		Cloneable {
	...
	/**
	 * Extract from the provided JsonParser. {"VC":[{"PID":"foo","LLC":[1]}]}
	 * 
	 * Parser should point at the START_OBJECT for the Vector Clock's
	 * representation will be returned at the END_OBJECT
	 * 
	 * @param jp
	 * @throws IOException
	...
	protected void fromJson(JsonParser jp) throws IOException {
		...
		if (jp.getCurrentName().compareTo(TextConstants.VECTOR_CLOCK) != 0)
			throw new JsonParseException("Unexpected field name: "
					...
					+ TextConstants.VECTOR_CLOCK, jp.getCurrentLocation());

		// start of Array;
		for (jp.nextToken(); jp.nextToken() != JsonToken.END_ARRAY;) {
			...
			if (jp.getCurrentName().compareTo(TextConstants.PROCESS_ID) != 0)
				throw new JsonParseException("Unexpected field name: "
						+ jp.getCurrentName() + " should be: "
						+ TextConstants.PROCESS_ID, jp.getCurrentLocation());
	...
	/**
	 * Constructor to build a VectorClock from the human-readable String created
	 * by toString()
	 * 
	 * 
	 * @param stringValue
	 *            the JSON string value of the vector clock
	 */
	...
	/**
	 * Update this Vector Clock with the values found in the argument. Vector
	 * Clocks are merged according to the following algorithm: For each key name
	 * in the argument clock: if this Vector does not contain the key, insert
	 * the key and a copy of the Logical Clock in this Vector. else update the
	 * value for the key in this vector to be the greater of the argument value
	 * and the current value.
	 */

	public synchronized void merge(VectorClock other) {

		for (String nm : other.clocks.keySet()) {

			if (!clocks.containsKey(nm)
					|| clocks.get(nm).isLessThan(other.clocks.get(nm))) {
	...
	protected void toJson(JsonGenerator jg) throws JsonGenerationException,
			IOException {

		jg.writeArrayFieldStart(TextConstants.VECTOR_CLOCK);

		for (String pid : clocks.keySet()) {
	...
	/**
	 * provide human and computer-readable JSON output. format is:
	 * {"VC":[{"PID":<process id>,<Logical Clock>},...]}
	 * 
	 */
	@Override
	...
	public Object clone() {
		...
		for (String wknm : this.clocks.keySet()) {
	...
	/**
	 * Logical Relation. This VectorClock is less than the parameter VectorClock
	 * if all of the following conditions are fulfilled:
	 *
	 * 1. No LogicalClock in the other vector is less than the corresponding
	 * LogicalClock in this Vector 2. At least one LogicalClock in this vector
	 * is less than the corresponding LogicalClock in the other vector.
	 *
	 ...
	 * 
	 * In order for THIS Vector Clock to be less than another, all of the
	 * following must be true: for every Logical Clock in common, none of the
	 * clocks in THIS vector can be greater than its counterpart for every
	 * Logical Clock in common, at least one in THIS must be less than its
	 * counterpart for every Logical Clock in THIS but absent from the other,
	 * the value in THIS must be zero.
	 */
	@Override
	protected boolean isLessThan(ClockOperations other) {
		...
		// is greater than the equivalent in other. Therefore, if we are less
		// than the
		// other by the comparison to this point, we are done.
		// If we have not found a 'less than' clock yet, we must search for a
		// clock in the other vector, that we do not have, that is greater than
		// zero.

		if (!lessThan) {
			for (String nm : ((VectorClock) other).clocks.keySet()) {

				if (!(clocks.containsKey(nm))
						&& zero.isLessThan(((VectorClock) other).clocks.get(nm)))
	...
	/**
	 * Increment the Logical Clock in this Vector belonging to the argument key.
	 */
